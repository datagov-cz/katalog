...
function preparePublishersInPlace(
  configuration,
  navigation,
  translation,
  publishers,
  ...
  for (const publisher of publishers) {
    ...
    publisher["dashboardDaily"] =
      configuration.client.publisherDashboardDailyTemplate.replace(
        "{}",
        publisher["iri"],
      );
    publisher["dashboardMonthly"] =
      configuration.client.publisherDashboardMonthlyTemplate.replace(
        "{}",
        publisher["iri"],
      );
    publisher.message = translation.translate(
      "datasets-found",
      publisher.count,
    );
    //
