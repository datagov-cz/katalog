...
export function parseClientQuery(navigation, query) {
  ...
  const keywordLimit = navigation.queryArgumentFromClient(
    query,
    "keyword-limit",
  );
  const formatLimit = navigation.queryArgumentFromClient(query, "format-limit");
  const hvdCategoryLimit = navigation.queryArgumentFromClient(
    query,
    "hvd-category-limit",
  );

  const vdfPublicData =
    navigation.queryArgumentFromClient(query, "vdf-public-data") === "1";
  const vdfCodelist =
    navigation.queryArgumentFromClient(query, "vdf-codelist") === "1";
  const hdfDataset =
    navigation.queryArgumentFromClient(query, "hdf-dataset") === "1";
  const dynamicData =
    navigation.queryArgumentFromClient(query, "dynamic-data") === "1";
  ...
  return {
    ...
    dataServiceType: navigation.queryArgumentArrayFromClient(
      query,
      "data-service-type",
    ),
